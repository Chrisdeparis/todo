...
func (t *Task) FindById(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.Println(err)
		handler.SendJSONError(w, "Not Found", http.StatusNotFound)
		return
	}
...
func (t *Task) Update(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.Println(err)
		handler.SendJSONError(w, "Error while decoding to update", http.StatusBadRequest)
		return
	}
	...
	handler.SendJSONWithStatus(w, nil, http.StatusNoContent)
}
